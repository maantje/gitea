...
func CreateOrUpdateSecret(ctx context.Context, ownerID, repoID int64, name, data string, protected bool) (*secret_model.Secret, bool, error) {
	...
	if len(s) == 0 {
		s, err := secret_model.InsertEncryptedSecret(ctx, ownerID, repoID, name, data, protected)
		if err != nil {
			return nil, false, err
		}
		return s, true, nil
